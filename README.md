...
> 上面这段演示 GIF 由 `tools/make_demo_gif.py` 无头录制：悬停看路径 → 点错丢一颗心 →
> 点对了飞出去 → 连锁清空 → 通关结算。
...
```bash
python tools/make_screenshots.py   # 11 张 PNG -> assets/
...
* 左上角：当前关卡序号（第 x / 9 关）与关卡名称；教学关则显示「教学关」
* 第 2 行横向三组数字：**剩余箭头数**、**生命值**（像素心）、**本关得分 / 本关满分**
...
│   ├── ui.py                 # 绘图工具：字体、文字折行、按钮、箭头、像素心与挂锁图标
│   ├── anim.py               # 动画效果：飞出、撞击抖动、飘字、心碎
│   ├── bgfx.py               # 动态背景：漂移光晕、上飘星点、流星、暗角
...
`python tests/test_game.py` 共 **94 个用例全部通过**，覆盖作业要求的 T01–T06：
...
| `GameFlowTestCase` | 完整流程、三个场景渲染、总览交互、教学引导、9 关顺序通关 |
