...
一个用 **Python + Pygame** 编写的点击式管道解谜小游戏。项目为软件工程课程第二次个人作业，
...
* 如果它**箭头**正前方的那条射线上**没有别的管道**，整条管道会沿着自己的方向飞出棋盘并被消除；
* 如果射线上**存在别的管道**，它不会消失，而是前冲一下再弹回、抖动、泛红，
  **并丢掉一颗心**；
* 清空本关全部管道即可通关，剩下的心越多，本关得分越高；心用光则本关失败，可以重新开始。

![游戏演示](assets/demo.gif)

> 上面这段演示 GIF 由 `tools/make_demo_gif.py` 无头录制：悬停看路径 → 开辅助线、
> 点一次提示 → 点错丢一颗心 → 点对了整条管道飞出去 → 连锁清空 → 通关结算。

**顺手给玩家配了三个小工具**（都摆在界面上，不用记快捷键）：

* **提示**：在「现在能飞出去的管道」里自动挑一支**消掉之后能解锁最多其它管道**的，
  套上一圈会呼吸的青绿高亮环，几秒后自己消失——不会一直挂着让人误以为那格有什么特殊状态；
* **辅助线**：把每支管道箭头方向的去路用虚线画出来（去路畅通的画到棋盘边，
...
管道从 **21 支增加到 46 支**。所有关卡都由程序校验过可解性。
...
> 关卡数据一改，写死的坐标会静默指向别的管道，截出来的图还是「能看」的，
> 只是已经不能说明它要说明的那件事了。
...
python tools/generate_levels.py -r 14 -c 10 -n 20 -k 6 --seed 123  # 自定义尺寸与管道数
python tools/pick_levels.py                                       # 从候选里挑出难度台阶
```

生成器用「逆向构造法」保证布局**必然可解**；`pick_levels.py` 再从同一尺寸的候选里
挑「铺满率最高 + 开局可点数最接近目标」的那一版。详见文件开头的说明。
...
| 操作 | 说明 |
| --- | --- |
| 鼠标左键点击管道 | 点它身上**任意一格**都算选中它；箭头前方空则整条飞出，被挡则丢一颗心 |
| 鼠标悬停在管道上 | 高亮显示它的前进路径：**绿色 = 畅通，红色 = 被挡**，红色停在挡路的那一格 |
| 底栏圆钮「提示」 | 高亮一支「点掉它最能解锁局面」的管道，几秒后自动消失（快捷键 `H`） |
| 底栏圆钮「辅助线」 | 开关；打开后给每支管道画出它箭头前方的去路（虚线，快捷键 `G`） |
...
* 下面一行四组信息横排，**整体居中**：🕐 **本关用时** · 生命值（像素心，`5 / 7`）·
  **剩余管道数** · **本关得分 / 本关满分**（教学关这一格写「不计分」）。
...
* 管道是一条**圆角粗管道**，末端一个三角箭头；颜色取自一组糖果色，
  但**相邻（上下左右）的两支管道不允许同色**——同色管道挨在一起会连成一片、数不清有几支；
* 悬停任意一格会显示它所属管道的去路；被挡时红段会停在挡路的那一格，再往后不画
  （后面的格子跟「为什么飞不出去」没有关系）。

**底部工具栏**

* 左边圆钮「提示」、中间缩放滑杆与百分比、右边圆钮「辅助线」（开关打开时圆钮和它下面的字都变成蓝色）
* 教学关额外有：目标管道上的黄色呼吸高亮环 + 底部的逐步讲解条

### 生命值怎么来的

生命值上限**按关卡难度星级给**，不是固定值：第 1 关 4 颗心，最后一关 7 颗心
（`game/levels.py` 的 `HP_BY_STARS`）。关卡越难、要扫的射线越多、越容易手滑，容错就给得越多，
...
于是每关满分正好是「星级 × 300」，9 关合计 **8400 分**。失败（心用光）本关 0 分，
...
│   ├── anim.py               # 动画效果：整条管道飞出、撞击抖动、飘字、心碎
...
3. **关卡即文本**：一支管道写成一行字符串，`行,列 方向 路径`，改关卡不用碰代码：

   ```python
   # "4,6 > D3R2"：从第 4 行第 6 列出发，箭头朝右，
   # 管道先向下 3 格、再向右 2 格，共 6 格；箭头在最后经过的那一格
   Level(
       name="交叉路口",
       hint="开局能点的很少，先找朝外的",
       max_hp=4,
...
| 关卡 | 名称 | 棋盘 | 管道数 | 占格 | 铺满率 | 开局可直接点掉的管道 | 生命值 | 难度 | 本关满分 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 第 1 关 | 初次拉弓 | 11×8 | 21 | 85 | 0.97 | 7 | ♥×4 | ★ | 300 |
| 第 2 关 | 交叉路口 | 13×9 | 23 | 116 | 0.99 | 6 | ♥×4 | ★ | 300 |
| 第 3 关 | 连锁反应 | 14×10 | 22 | 136 | 0.97 | 5 | ♥×5 | ★★ | 600 |
| 第 4 关 | 四面楚歌 | 16×11 | 27 | 170 | 0.97 | 5 | ♥×6 | ★★★ | 900 |
| 第 5 关 | 错位走廊 | 18×12 | 28 | 210 | 0.97 | 4 | ♥×6 | ★★★ | 900 |
| 第 6 关 | 纵横交错 | 20×14 | 37 | 269 | 0.96 | 4 | ♥×6 | ★★★★ | 1200 |
| 第 7 关 | 长蛇阵 | 22×15 | 38 | 313 | 0.95 | 3 | ♥×6 | ★★★★ | 1200 |
| 第 8 关 | 十面埋伏 | 23×17 | 43 | 372 | 0.95 | 3 | ♥×7 | ★★★★★ | 1500 |
| 第 9 关 | 万箭归一 | 26×18 | 46 | 430 | 0.92 | 3 | ♥×7 | ★★★★★ | 1500 |

> 满分合计 **8400**。第 1~4 关是手工设计的小棋盘；第 5~9 关由 `tools/generate_levels.py`
> 逆向构造生成，再用 `tools/pick_levels.py` 从候选里挑出来（同一尺寸挑「铺满率最高 +
> 开局可点数最接近目标」的那一版）。
>
...
这个玩法有个容易被忽略的性质：点掉一支能飞出去的管道，只会让其它管道的射线更空，
所以**任何一步正确操作都不会把自己玩死**——不存在「顺序错误」，
唯一会输的方式是点了一支其实被挡住的管道。
因此难度只来自一件事：**在棋盘上找出此刻哪些管道的前方是空的**。

* **开局可直接点掉的管道越少**，越要在开局就仔细扫视；**棋盘越大、管道越多**，
  需要顺着看的行与列越多；
* 真正决定手感的是**开局可点数**：从第 1 关的 7 支一路收到第 7 关之后的 3 支；
* 生命值上限随难度递增，是「容错」而不是「惩罚」：前期容错给足，让玩家敢试。

难度分按 `管道数 × 1.6 + 棋盘格数 × 0.12 − 开局可点数 × 3.0` 计算，
再按阈值 `(<35, <45, <70, <100, 其余)` 映射成 1~5 颗星。
九个关卡的难度分是 23.2 / 32.8 / 37.0 / 49.3 / 58.7 / 80.8 / 91.4 / 106.7 / 120.8，
一路单调递增，测试里有一条用例卡着这个阶梯。
...
`python tests/test_game.py` 共 **222 个用例全部通过**，覆盖作业要求的 T01–T06：

| 编号 | 测试内容 | 预期结果 | 实际结果 |
| --- | --- | --- | --- |
| T01 | 点击前方无阻挡的管道 | 整条管道飞出棋盘并消失 | ✅ 通过 |
| T02 | 点击前方有阻挡的管道 | 管道不消失，生命值减 1 | ✅ 通过 |
| T03 | 点击箭头朝向棋盘外的管道 | 管道正常消失，不发生越界错误 | ✅ 通过 |
| T04 | 消除本关全部管道 | 显示通关并进入下一关 | ✅ 通过 |
| T05 | 生命值耗尽 | 显示失败并允许重新开始 | ✅ 通过 |
| T06 | 游戏进行中重新开始 | 管道布局和生命值恢复 | ✅ 通过 |

测试分成十组：

| 测试类 | 覆盖内容 |
| --- | --- |
| `BoardRuleTestCase` | 棋盘规则、边界、点击管道任意一格、生命值、胜负判定、`reset` |
...
| `RenderPrimitiveTestCase` | 文字折行与宽度、按钮命中、滑杆取值、像素心图标 |
| `AnimationTestCase` | 整条管道飞出、撞击抖动、飘字与心碎的生命周期 |
| `BackgroundTestCase` | 背景确实在动、暗角与光晕不越界 |
| `GameFlowTestCase` | 完整流程、场景渲染、总览交互、教学引导、9 关顺序通关、计时 / 提示 / 辅助线 / 缩放平移、界面三块分区不重叠、辅助线几何（`guide_segment` 与被挡路径共线且落在挡路那一格内、管道飞走后消失） |
| `VisualVarietyTestCase` | 相邻管道不同色、日夜间主题两套配色都完整、管道与箭头造型、背景元素 |

另有边界用例：点到空格子不丢心、点到棋盘外不报错、本关结束后点击无效、
死锁布局被判定为无解、非法关卡布局直接报错（含「箭头正对自己的管道」）、
教学关缺步骤直接报错等。
...
**Q4：管道太细看不清？**
用底栏的缩放滑杆放大（最高 180%），放大后按住棋盘可以拖动。管道粗细与箭头大小的比例
在 `game/config.py` 的 `PIECE_STROKE_RATIO` / `PIECE_HEAD_RATIO` 里，改完重跑一次截图脚本即可。

**Q5：想自己加关卡？**
在 `game/levels.py` 的 `LEVELS` 里加一个 `Level(...)`（管道写成 `"行,列 方向 路径"`），
