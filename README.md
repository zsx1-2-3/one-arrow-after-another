...
界面底子是**会动的**：斜向的极光带缓缓流过、两团光晕慢慢游走、浮尘上浮轻摆，
隔一会儿还划一颗流星。整体亮度压得很低——背景是衬托不是主角，棋盘上的箭头才是。
日间主题下光源换成「减法」：同一套贴图变成淡淡的**云影**缓缓飘过。
四层动效的浓淡都在 `config.BG_*` 里，哪一层不想要把数量写 0 就是。
...
│   ├── bgfx.py               # 动态背景：极光带 + 漂移光晕 + 浮尘 + 流星 + 暗角
...
8. **界面分区由常量推导，不靠眼睛量**：信息栏 / 棋盘 / 工具栏的高度各是一个常量，
   ...
   不用去 9 个关卡里逐关截图看。
9. **背景是四层预渲染贴图，动画里只做 blit**：`bgfx.py` 的极光带、光晕、浮尘、流星
   一律启动时算成贴图 + 按亮度分档缓存，每帧只叠加一次，不做逐像素运算
   （无头实测单帧 1.5ms 左右）。两处实打实的取舍：
   光带是「又宽又扁」的一条，`transform.rotate` 按外接矩形收费、白白撑大四倍
   （实测 21MB），改成逐列错位倾斜之后落到 8MB；
   日间主题底子是浅色，加法光只会糊成白斑，于是同一张贴图改用**减法**叠加，
   就成了「云影缓缓飘过」——换的只是 blit 时那个 `special_flags`。
