...
2026-09-22 的调整：
    按参考图重做观感之后，底子要求「干净」——原版的星点和流星在深紫底上
    显得很碎，而且会和棋盘上的箭头抢注意力。所以星点数量与流星间隔都改成了
    "相当于不出现"（见 config.BG_STAR_COUNT / BG_SHOOT_INTERVAL），
    只留下两团几乎看不出在动的光晕，让整块底不是死板的纯色。
    相关代码保留着：想恢复"星空"只要把那两个配置改回去。
...
观感上的一个约束：
    背景是衬托，不是主角。光晕压得很暗，叠加上去之后棋盘区域依然要保持对比度，
    不能出现"背景比箭头还抢眼"的情况。
"""
...
# 光晕的配色：偏冷的紫 / 青，和整屏的深紫底子同一色系。
# 注意整体压得比原版暗——底色本身已经提亮了（(63,65,85) 而不是 (24,30,52)），
# 光晕再亮就会把底色顶成一块脏斑。
ORB_COLORS = (
    (58, 52, 108),
    (40, 84, 106),
