...
# ---- 动态背景 ----
# 背景是四层动效叠出来的，全部走「预渲染贴图 + 每帧只 blit」，不做逐像素运算：
#   1. 极光带   —— 几条斜向柔光带缓缓往下流过。夜间是加法发光；日间主题底子浅，
#                  加法只会糊成白斑，所以同一张贴图改用减法叠加，成了"云影飘过"。
#   2. 漂移光晕 —— 两团压得很暗的柔光，沿李萨如曲线慢慢游走，几乎看不出在动。
#   3. 浮尘     —— 缓慢上浮、左右轻摆、明灭呼吸的小光点（就是早年的"星点"，
#                  数量与亮度都往下压过，免得深紫底上显得碎、和棋盘抢注意力）。
#   4. 流星     —— 隔十几二十秒划一颗，给一直平着的画面一点惊喜。
# 数量写 0 就是关掉那一层；游戏界面（SCENE_PLAY）一律比菜单 / 关卡页收敛。
BG_AURORA_COUNT = 6             # 极光带条数
BG_AURORA_SPEED = (9.0, 22.0)   # 每条的下沉速度区间（像素/秒，全屏约 40~90 秒走完）
BG_AURORA_DRIFT = 1.0           # 极光带整体浓淡（游戏界面会再乘 BG_PLAY_DIM）
BG_AURORA_LEVELS = 4            # 亮度分档数：呼吸只在档位之间跳，贴图缓存撑不爆
BG_AURORA_PAD_RATIO = 0.16      # 屏幕上下各留这么多高度当"屏外"，带子从这儿进出
BG_STAR_COUNT = 30              # 浮尘数量
BG_STAR_COUNT_PLAY = 14
BG_STAR_SPEED = (6.0, 20.0)     # 上浮速度区间（像素/秒）
BG_STAR_SWAY = (5.0, 20.0)      # 左右摆动幅度（像素）
BG_ORB_COUNT = 2
BG_ORB_SPEED = 0.045
BG_PLAY_DIM = 0.45              # 游戏界面里极光带的浓淡（别和棋盘抢注意力）
BG_DAY_DIM = 0.55               # 日间主题下云影的浓淡（浅底上减法要收着点）
BG_SHOOT_INTERVAL = (12.0, 26.0)        # 流星间隔区间（秒）
BG_SHOOT_INTERVAL_PLAY = (26.0, 55.0)
