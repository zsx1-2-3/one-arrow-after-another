...
# ---- 生命值（点错一次扣 1 点）----
COLOR_HP = (240, 98, 98)               # 还有的生命值（实心像素心）
COLOR_HP_LOW = (246, 178, 72)          # 只剩最后 1 点时换成提醒色
COLOR_HP_LOST = (88, 64, 78)           # 已经失去的生命值（暗色，只剩轮廓）

# ---- 得分 ----
# 得分数字用金色，和难度星星同一个色系：都是「你赚到了什么」的信息，
# 和生命值（红）、剩余箭头（蓝）区分开，HUD 上一眼能分清三组数字。
COLOR_SCORE = (250, 206, 96)
COLOR_STAT_BOX = (40, 49, 76)          # 结果面板里的数据格
COLOR_STAT_BOX_SCORE = (58, 51, 30)    # 结果面板里的「本关得分」格（暖色调）
...
#
# 四个颜色的**感知亮度**（0.299R + 0.587G + 0.114B）都锁在 174 左右
# （分别是 173.4 / 175.2 / 173.9 / 173.5），所以整屏箭头看起来是「一套」的，
# 不会有的刺眼、有的发闷。改色时请一并维持这个亮度，否则又会出现明暗跳变。
#
...
# 箭头贴图内部的竖向光泽：顶部略提亮、底部略压暗，只为了有立体感。
# 幅度刻意压得比较小，避免同一支箭头自己就上亮下暗看着不统一。
ARROW_HIGHLIGHT = 0.22    # 箭头贴图顶部提亮比例
ARROW_SHADE = 0.16        # 箭头贴图底部压暗比例
