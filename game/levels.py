...
教学关不算一关
--------------
教学关单独放在 TUTORIAL 里，**不在 LEVELS 里**：它不计分、不占关卡编号、
不需要解锁，主菜单上有单独入口，随时可以回去复习。
这样编号就从第 1 关「初次拉弓」干净地开始，
玩家也不会把「教学关」误当成一道要拿分、要解锁的题。

前四关的难度阶梯
----------------
第 1~4 关是手工设计的小棋盘，难度一路上升，而且"难"的来源逐关叠加：

    第 1 关  5×5   4 支   3 支能直接飞   把「前方空就能飞」自己走一遍
    第 2 关  5×5   4 支   只有 1 支能飞   要顺着链条一支一支解
    第 3 关  7×6  10 支   棋盘变大、箭头变多
    第 4 关  7×7  12 支   尺寸封顶前的最后一步

第 6~9 关由 tools/generate_levels.py **逆向构造**生成：
把「消除顺序」倒过来当「放置顺序」，放置时要求"新箭头正前方没有已放置的箭头"，
这样「放置顺序的逆序」天然就是一条合法通关顺序，布局必然可解。

难度从第 6 关起换成靠**提高密度**——棋盘尺寸到第 8 关就封顶在 9×9，
之后同样的格子里塞进更多箭头。注意这件事有陷阱：贴着边、朝棋盘外的箭头
射线长度是 0，永远能飞，不管它的话密度一高开局可点数反而暴涨（比稀疏时还简单）。
生成器因此会避开这类候选，并用 `seen × 1 + raylen × 3` 打分，
让高密度布局仍能把开局可点数压在 3~5 支。详见 tools/generate_levels.py 开头。

九个关卡的箭头数是 4 / 4 / 10 / 12 / 18 / 24 / 30 / 40 / 50，
每加密一档只多 6~10 支；棋盘则是 5×5 → 6×7 → 7×7 → 8×8 → 9×9 一级一级放大，
放满之后才交给密度。相邻两关的难度分差从 4.0 一路拉到 17.7，
不是匀着加，而是越往后加得越多——前面留足台阶让人上手，后面才开始认真。
...
看着既单调又显简单（实测第 7 关相邻同向对占 59%、最大同色块 6 格）。
...
（只有第 4 关的「只有一支能飞」松到两支，hint 已同步）。
第 6 关是在生成器里直接按这条线筛出来的：25% 正好是四个方向完全交错的理论值。
...
教学关不参与得分，也**不走这张表**：它单独给 6 颗心，是个随便点的沙盒，
不该让人在教程里被判失败。
...
#
# 只有 LEVELS 里的编号关卡走这张表；教学关（TUTORIAL）不参与计分，
# 单独给 6 颗心，见下面的说明。
...
class Level:
    ...
    def stars(self):
        ...
        阈值是按当前 9 个编号关卡的实际难度分挑的，让星级均匀铺开
        （5.2 → 1 星，9.2 → 2 星，19.1 / 22.6 → 3 星，
        33.8 / 43.7 / 56.1 → 4 星，70.8 / 88.5 → 5 星）。
        """
...
#  主菜单上有单独入口（也随时可以回去复习）。
#  生命值给得宽松：教程的作用是让人放胆去点，而不是在这里被判失败。
TUTORIAL = Level(
    name="教学关",
    hint="跟着黄色提示点，很快就能上手",
    max_hp=6,
...
LEVELS = (
    # ======================================================== 第 1 关
    #  教学关之后的第一次单飞：4 支箭头里有 3 支前方是空的，
    #  顶上那支朝下的箭头被底部的箭头挡住——把「前方空就能飞」自己走一遍。
    Level(
        name="初次拉弓",
        hint="哪支箭头前方是空的，就先点哪支",
        max_hp=4,
        layout=(
            "..v..",
            ".....",
    ...
    # ======================================================== 第 2 关
    #  同样是 5×5，但全场只剩一支能直接飞：要顺着链条一支一支解。
    Level(
        name="交叉路口",
    ...
    # ======================================================== 第 5 关
    #  第 6~9 关由 tools/generate_levels.py 生成（逆向构造，布局必然可解），
    #  再用 tools/deshuffle_levels.py 把同色扎堆磨平：保持位置不动、只逐个试换方向，
    #  要求「开局可点数不变 + 换完仍可解」，所以难度没被顺手改掉。
    #  刻意**不靠放大棋盘**加难：尺寸到第 8 关就封顶在 9×9，
    #  之后只靠提高密度（同样的格子塞进更多箭头）继续变难。
    #  第 5~9 关的箭头数 18 / 24 / 30 / 40 / 50，密度 0.37 / 0.38 / 0.47 / 0.49 / 0.62。
    Level(
        name="错位走廊",
    ...
    # ======================================================== 第 6 关
    #  棋盘第一次到 8×8：格子多了，但箭头只比上一关多 6 支，
    #  所以密度反而略降——先让人适应更大的视野，下一关才开始挤。
    #  开局能直接飞的只有 3 支，方向分布极匀（相邻同向对 25%，是理论最均匀值）。
    Level(
        name="纵横交错",
        hint="棋盘大了一圈，四个方向都要扫一遍再动手",
        max_hp=6,
        layout=(
            ".v.>^.v.",
            ">.v.....",
