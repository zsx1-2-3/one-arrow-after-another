...
棋盘尺寸到第 9 关就封顶在 9×9，之后同样的格子里塞进更多箭头。
...
class Level:
    ...
    def stars(self):
        ...
        （-2.2 / 2.0 → 1 星，3.2 / 13.1 → 2 星，20.1 / 27.8 → 3 星，
        39.2 / 51.6 / 56.5 → 4 星，67.8 以上 → 5 星）。
        """
        score = self.difficulty_score
        for threshold, stars in ((3.0, 1), (17.0, 2), (33.0, 3), (62.0, 4)):
            if score < threshold:
                return stars
        return 5
...
LEVELS = (
    ...
    Level(
        ...
        steps=(
            TutorialStep("先点这支朝右的箭头。它前方还有一支箭头挡路，飞不出去，"
                         "会扣掉 1 点生命值——这就是「被挡住」的样子。", 1, 1, "blocked"),
    ...
    # ======================================================== 第 6 关
    #  第 6~12 关由 tools/generate_levels.py 生成（逆向构造，布局必然可解）。
    #  这里刻意**不靠放大棋盘**加难：尺寸到第 9 关就封顶在 9×9，
