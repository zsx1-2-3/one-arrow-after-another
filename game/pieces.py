# -*- coding: utf-8 -*-
"""棋子模型：一支「箭」是一条占多格的折线管道，末端带一个箭头。

参照同类游戏的实际画面，箭头**不是「一格一支」**，而是一条蜿蜒的管道：
...
规则和单格箭头完全一样：从「头」（管道末端）沿着箭头方向看出去，
...
def faces_own_body(piece, rows, cols):
    ...
    为什么专门判这一条：管道是从箭头往回长的，绕一圈之后完全可能把尾巴
    甩到箭头正前方去——那是「箭头指着自己的管子」，画面上像个死结，
    ...
    **末端的一段必须和箭头方向一致**（否则画出来的箭头会歪在拐角上），
    ...
    for index, piece in enumerate(pieces):
        ...
        if faces_own_body(piece, rows, cols):
            raise ValueError("第 %d 支箭的箭头正对着自己的管道（会永远飞不出去）" % index)
    return True
