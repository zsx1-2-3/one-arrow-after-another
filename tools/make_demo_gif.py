...
录制的路线（脚本里每一步都有注释，这里只说叙事）：
    ...
    开始界面 → 第 2 关 → 点掉一两支（看整条管道飞出去）
    → 悬停被挡的管道（红路径）→ 悬停通畅的管道（绿路径）
...
def main():
    ...
    # 选「交叉路口」：13×9 的棋盘在 GIF 的宽度下还看得清每支管道，
    # 开局能直接点的管道不多（5~7 支），正好演示「扫射线找出口」这件事。
    ...
    # -------- 4. 悬停在一支被挡住的管道上：红色路径 + 挡路那格套红框 --------
    blocked = find_blocked(game.board)
    hover(game, blocked)
    record(game, screen, frames, 0.9)

    # -------- 5. 悬停在一支能飞出去的管道上：绿色路径一路画到盘外 --------
    free = find_free(game.board)
    hover(game, free)
    record(game, screen, frames, 0.9)

    # -------- 6. 打开辅助线 + 用一次提示 --------
    # 辅助线把每支管道的去路都画出来，提示再直接指出先点哪一支。
    game.toggle_guides()
